...
Nova rodada solicitada em áudio em 24/09/2026: tabela de colaboradores no dashboard para distribuir demandas, filtro por profissão, contagem atribuída e criação central com cliente pesquisável, pauta e responsável. O exemplo de seis demandas frente a uma referência de doze orienta a decisão humana; não solicita quota automática ou bloqueio. Implementação concluída e QA próprio aprovado em 80 testes unitários e 53 cenários de API/navegador; publicação no Preview em andamento. A reparação dos registros reais foi autorizada posteriormente e concluída em uma ação separada da implementação do dashboard; sua verificação não substitui o QA da nova função. Evidência própria: `evidence/team-workload-20260924.json`.
