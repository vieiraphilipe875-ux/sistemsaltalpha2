...
A interface confirma alterações após a resposta do servidor, atualiza a área de trabalho a cada 15 segundos enquanto a aba está visível e ao recuperar foco. Não há edição colaborativa simultânea de texto: se duas pessoas editarem a mesma pauta ao mesmo tempo, a última gravação pode prevalecer. Controle de versões da pauta e resolução de conflitos são uma etapa posterior; arquivos finais já possuem versões.
