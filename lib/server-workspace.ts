import { defaultKanbanColumns, kanbanScopeKey, type KanbanBoardConfig, type KanbanKind } from "./kanban";
...
export async function getWorkspaceData():Promise<WorkspaceData|null> {
 ...
 const savedKanban = await db.select().from(kanbanBoards).where(eq(kanbanBoards.agencyId, agencyId));
 const findKanban = (kind: KanbanKind, clientId: string | null = null) => savedKanban.find(config => config.id === kanbanScopeKey(agencyId, kind, clientId));
 const makeKanban = (kind: KanbanKind, clientId: string | null = null): KanbanBoardConfig => {
  const config = findKanban(kind, clientId);
  return { kind, clientId, revision: config?.revision ?? 0, columns: config?.columns ?? defaultKanbanColumns(kind) };
 };
 const cardColumn = (kind: KanbanKind, columnId: string | null, status: string, clientId: string | null = null) => {
  const config = findKanban(kind, clientId);
  return config ? config.columns.find(column => column.id === columnId)?.id ?? null : columnId ?? status;
 };
 return {
  ...
  clients:visibleClients.map(c=>{
    ...
    return {...client,columnId:crm?cardColumn("crmClients",c.columnId,c.status):null,...(finance?{revenue,dueDay}:{}),contactName:crm?c.contactName:"",phone:crm?c.phone:"",email:crm?c.email:"",notes:crm?c.notes:"",avatarUrl:c.avatarKey?`/api/clients/${c.id}/media?kind=avatar&v=${encodeURIComponent(c.avatarKey)}`:null,bannerUrl:c.bannerKey?`/api/clients/${c.id}/media?kind=banner&v=${encodeURIComponent(c.bannerKey)}`:null};
  }),
  ...
  deliverables:tasks.map(t=>({...t,columnId:cardColumn("demands",t.columnId,t.status,allBoards.find(b=>b.id===t.boardId)!.clientId),slides:allSlides.filter(s=>s.deliverableId===t.id),assets:allAssets.filter(a=>a.deliverableId===t.id).map(a=>({...a,url:`/api/assets/${a.id}`})),attachments:allAttachments.filter(a=>a.deliverableId===t.id).map(a=>({...a,url:`/api/attachments/${a.id}`})),references:allReferences.filter(r=>r.deliverableId===t.id)})),
  ...
  crmLeads:crm?(await db.select().from(crmLeads).where(eq(crmLeads.agencyOwnerId,agencyId))).map(lead=>({...lead,columnId:cardColumn("crmLeads",lead.columnId,lead.status)})):[],
  crmDeals:crm?(await db.select().from(crmDeals).where(eq(crmDeals.agencyOwnerId,agencyId))).map(deal=>({...deal,columnId:cardColumn("crmDeals",deal.columnId,deal.stage)})):[],
