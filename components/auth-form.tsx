...
export function AuthForm({initialView="login",invite="",resetId="",resetToken=""}:{initialView?:"login"|"signup"|"verify"|"forgot"|"reset";invite?:string;resetId?:string;resetToken?:string}){
 ...
 function change(next:typeof view){setView(next);setError("");setMessage("");setPassword("");setConfirmation("");}
 async function submit(e:React.FormEvent){e.preventDefault();setBusy(true);setError("");setMessage("");try{
  ...
  else if(view==="signup"){setView("verify");setPassword("");setConfirmation("");setMessage(result.message||"Enviamos um código para confirmar seu e-mail.");}
 ...
 return <form className="auth-form" onSubmit={submit}>
  ...
  {view==="verify"&&<button className="text-action" type="button" disabled={busy||!email} onClick={async()=>{setBusy(true);setError("");try{const r=await call("resend",{email});setMessage(r.message);}catch(e){setError((e as Error).message);}finally{setBusy(false);}}}>Reenviar código</button>}
