...
export function ProfileDialog({open,onOpenChange,data,reload}:{open:boolean;onOpenChange:(v:boolean)=>void;data:WorkspaceData;reload:()=>Promise<void>}){
 ...
 return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent><DialogHeader><DialogTitle>Seu perfil</DialogTitle><DialogDescription>Seu nome e profissão acompanham sua conta em todas as agências.</DialogDescription></DialogHeader><label>Nome<Input value={name} onChange={e=>setName(e.target.value)}/></label><label>Profissão<select className="profile-select" value={profession} onChange={e=>setProfession(e.target.value)}>{Object.entries(professionLabels).map(([k,v])=><option key={k} value={k}>{v}</option>)}</select></label><p className="text-sm text-muted-foreground">{data.currentMember.email}</p><DialogFooter><Button disabled={busy||name.trim().length<2} onClick={save}>{busy?"Salvando...":"Salvar perfil"}</Button></DialogFooter></DialogContent></Dialog>;
}
