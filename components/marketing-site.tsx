...
export function MarketingSite() {
  return <div id="postito-landing" className="landing">
    ...
    <main id="conteudo">
      <section className="landing-hero landing-container">
        <div className="landing-overline"><span>Feito para a rotina das agências</span><span>Do primeiro contato à entrega</span></div>
        <h1><span className="hero-line"><span data-hero-line>Sua agência.</span></span><span className="hero-line"><span data-hero-line>Em boa <span className="hero-order">ordem<svg viewBox="0 0 356 18" fill="none" aria-hidden="true"><path d="M3 14C87 2 239 2 351 10" stroke="currentColor" strokeWidth="7" strokeLinecap="round" /></svg></span>.</span></span></h1>
        <div className="landing-hero-bottom"><p>Boas ideias precisam de espaço.<br />Reúna demandas, clientes, equipe e financeiro. Deixe o trabalho fluir.</p><div className="landing-hero-actions"><Link className="landing-button" href="/cadastro">Começar com o Postito <ArrowUpRight size={20} aria-hidden="true" /></Link><a className="landing-text-link" href="#plataforma">Conhecer por dentro <ArrowDown size={18} aria-hidden="true" /></a></div></div>
        <figure className="hero-product"><div className="product-frame-caption"><span><Layers2 size={16} aria-hidden="true" /> Um lugar para a agência inteira</span><span>01 / Visão geral</span></div><Image src="/marketing/visao-geral.webp" alt={views[0].alt} width={1600} height={1050} priority sizes="(max-width: 700px) 100vw, 90vw" /><figcaption>Interface real. Dados de demonstração.</figcaption></figure>
        <div className="landing-module-strip" aria-label="Áreas da plataforma"><span>Demandas & pautas</span><Plus aria-hidden="true" /><span>Clientes & equipe</span><Plus aria-hidden="true" /><span>CRM & financeiro</span><span className="module-result">Tudo conectado <ArrowUpRight size={18} aria-hidden="true" /></span></div>
      </section>
      ...
      <section className="landing-platform landing-container" id="plataforma" aria-labelledby="platform-heading"><div className="landing-section-heading" data-reveal><p className="landing-label">02 / Conheça a plataforma</p><h2 id="platform-heading">Toda a operação.<br /><span>Uma visão em comum.</span></h2><p>Entre nas áreas do Postito e veja como cada parte do trabalho se conecta.</p></div>
      ...
      <section className="landing-flow landing-container" aria-labelledby="flow-heading"><div className="landing-section-heading" data-reveal><p className="landing-label">03 / Da ideia à entrega</p><h2 id="flow-heading">Cada pessoa sabe<br />o próximo passo.</h2></div><div className="landing-flow-grid">{[
        ["01", "Organize o contexto.", "Crie a pasta do cliente, defina a pauta e mantenha o briefing e os arquivos perto da demanda.", "Cliente → Pauta"],
        ["02", "Conecte as pessoas.", "Atribua responsáveis, acompanhe a distribuição de trabalho e libere os clientes certos para cada pessoa.", "Equipe → Produção"],
        ["03", "Acompanhe a entrega.", "Veja o que está em criação, em revisão e aprovado. Continue a conversa no contexto do trabalho.", "Revisão → Aprovação"],
      ].map(([number,title,description,tag])=><article className="landing-flow-item" data-reveal key={number}><span className="flow-number">{number}</span><h3>{title}</h3><p>{description}</p><span className="flow-tag">{tag}<ArrowUpRight size={17} aria-hidden="true" /></span></article>)}</div></section>

      <section className="landing-difference" id="diferenciais" aria-labelledby="difference-heading"><div className="landing-container"><div className="landing-section-heading" data-reveal><p className="landing-label">04 / Por que Postito</p><h2 id="difference-heading">O valor está<br />nas conexões.</h2><p>Quando a operação fica espalhada, você precisa reunir o contexto. No Postito, as informações acompanham o trabalho.</p></div><div className="difference-table" data-reveal role="table" aria-label="Ferramentas separadas e operação conectada no Postito"><div role="row" className="difference-table-head"><div role="columnheader">Na rotina</div><div role="columnheader">Em ferramentas separadas</div><div role="columnheader">Com o Postito</div></div>{[
      ...
      <section className="landing-faq landing-container" id="duvidas" aria-labelledby="faq-heading"><div data-reveal><p className="landing-label">05 / Antes de começar</p><h2 id="faq-heading">Vamos deixar<br />tudo claro.</h2></div><div className="faq-list">{faqs.map(([question,answer])=><details key={question} data-reveal><summary>{question}<Plus size={20} aria-hidden="true" /></summary><p>{answer}</p></details>)}</div></section>

      <section className="landing-final"><div className="landing-container" data-reveal><p className="landing-label">Seu próximo projeto começa aqui</p><h2>Dê espaço<br />às boas ideias.</h2><Link href="/cadastro" className="landing-button">Criar minha conta <ArrowUpRight size={20} aria-hidden="true" /></Link><Link href="/login" className="landing-text-link">Já tenho conta <ArrowRight size={18} aria-hidden="true" /></Link></div></section>
    </main>
    <footer className="landing-footer landing-container"><div className="footer-top"><p>Organização para fazer<br />o trabalho fluir.</p><nav aria-label="Rodapé"><a href="#plataforma">Plataforma</a><a href="#diferenciais">Diferenciais</a><Link href="/login">Entrar</Link><Link href="/cadastro">Criar conta</Link></nav></div><div className="footer-wordmark" aria-hidden="true">postito</div><div className="footer-bottom"><span>© {new Date().getFullYear()} Postito</span><span>Sua agência, com tudo no lugar.</span><a href="#conteudo">Voltar ao início <ArrowUpRight size={14} aria-hidden="true" /></a></div></footer>
    <MarketingMotion />
  </div>;
}
