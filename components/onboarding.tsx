...
export function Onboarding({name,invite,agencies}:{name:string;invite?:string;agencies:{id:string;name:string}[]}){
 ...
 {invite?<><button className="primary-action" disabled={busy} onClick={()=>act({action:"acceptInvite",token:invite})}><Check size={18}/>Aceitar convite</button><Link className="text-action" href="/">Ir para minhas agências</Link></>:<><form onSubmit={e=>{e.preventDefault();const fields=new FormData(e.currentTarget);void act({action:"createAgency",name:String(fields.get("agency")??"").trim()});}}><label>Nome da agência<input name="agency" required minLength={2} maxLength={100} disabled={busy} placeholder="Como sua agência se chama?"/></label><button className="primary-action" disabled={busy}>{busy?"Criando agência...":"Criar agência"} <ArrowRight size={18}/></button></form>{agencies.length>0&&<div className="agency-list">{agencies.map(a=><button key={a.id} disabled={busy} onClick={()=>act({action:"switchAgency",agencyId:a.id})}><Building2 size={18}/>{a.name}<ArrowRight size={16}/></button>)}</div>}</>}
