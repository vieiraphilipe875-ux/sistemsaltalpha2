...
CREATE TABLE `finance_workers` (
	...
	`invoice_required` integer DEFAULT false NOT NULL,
	`contract_end` text,
...
CREATE TABLE `worker_competencies` (
	...
	`created_at` text NOT NULL,
	`updated_at` text NOT NULL,
	FOREIGN KEY (`worker_id`) REFERENCES `finance_workers`(`id`) ON UPDATE no action ON DELETE cascade
);
...
CREATE TABLE `financial_documents` (
	...
	FOREIGN KEY (`worker_competency_id`) REFERENCES `worker_competencies`(`id`) ON UPDATE no action ON DELETE cascade,
